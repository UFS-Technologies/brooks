...
async function updateMenu() {
    ...
    try {
        ...
        // 1. Insert Enquiry Source into menu table if not exists
        const [insertMenu] = await connection.query(`
            INSERT INTO menu (Menu_Name, Route, Parent_Menu_ID, Delete_Status)
            SELECT 'Enquiry Source', '/admin/Enquiry_Source', NULL, 0
            FROM DUAL
            WHERE NOT EXISTS (SELECT 1 FROM menu WHERE Route = '/admin/Enquiry_Source');
        `);
        console.log("Insert Menu Result:", insertMenu);

        // 2. Get the Menu_ID for Enquiry Source
        const [menuRows] = await connection.query("SELECT Menu_ID FROM menu WHERE Route = '/admin/Enquiry_Source'");
        if (menuRows.length === 0) {
            console.error("Failed to find Enquiry Source menu after insertion");
            await connection.end();
            return;
        }
        const enquirySourceMenuId = menuRows[0].Menu_ID;
        console.log("Enquiry Source Menu ID:", enquirySourceMenuId);
        ...
        for (const user of adminUsers) {
            ...
            `, [enquirySourceMenuId, user.User_ID, enquirySourceMenuId, user.User_ID]);
        }
