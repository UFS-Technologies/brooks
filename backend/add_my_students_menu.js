...
async function setup() {
    try {
        ...
        const route = 'My_Students';

        // 1. Insert My Students menu into menu table if not exists
        const [insertMenu] = await new Promise((resolve, reject) => {
            db.query(`
                INSERT INTO menu (Menu_Name, Route, Parent_Menu_ID, Delete_Status)
                SELECT ?, ?, NULL, 0
                FROM DUAL
                WHERE NOT EXISTS (SELECT 1 FROM menu WHERE Menu_Name = ?);
            `, [menuName, route, menuName], (err, results) => {
                if (err) reject(err);
        ...
        // 2. Get the Menu_ID for My Students
        const menuRows = await new Promise((resolve, reject) => {
            db.query("SELECT Menu_ID FROM menu WHERE Menu_Name = ?", [menuName], (err, results) => {
                if (err) reject(err);
                else resolve(results);
            });
        });
        ...
        if (menuRows.length === 0) {
            console.error("Failed to find 'My Students' menu after insertion");
            process.exit(1);
        }
        ...
        for (const user of adminUsers) {
            await new Promise((resolve, reject) => {
                ...
                `, [menuId, user.User_ID, menuId, user.User_ID], (err, results) => {
                    if (err) reject(err);
                    else resolve(results);
                });
            });
